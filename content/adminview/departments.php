<?php
// content/departments.php
// This file contains the Department Management section content for admin_panel.php.
// It assumes $departments and $error_message are available from header.php.
...
if (!isset($conn) || !$conn instanceof mysqli) {
    error_log("Database connection (MySQLi) failed in departments.php.");
...
// Fetch total count of departments for pagination
// Assuming 'status = 1' for active departments, similar to colleges.php
$totalResult = $conn->query("SELECT COUNT(*) as total FROM departments WHERE status = 1");
$totalDepartments = $totalResult->fetch_assoc()['total'];
$totalPages = ceil($totalDepartments / $itemsPerPage);

// Prepared statement for fetching departments with pagination
// Join with colleges table to get college name if 'college_name' is not directly in departments table
$stmt = $conn->prepare("SELECT d.*, c.college_name FROM departments d LEFT JOIN colleges c ON d.college_id = c.college_id WHERE d.status = 1 ORDER BY d.department_id ASC LIMIT ?, ?");
...
/* Column widths for fixed table layout - ADAPTED FOR DEPARTMENTS */
...
<div class="admin-section" id="departments">
    <h2 class="h3 fw-bold">Departments Management</h2>
    ...
    <div class="action-and-per-page-row">
        ...
        <div class="action-buttons">
            <a href="<?php echo base_url . 'admin_dashboard.php?section=add_department'; ?>"
                class="btn btn-success btn-sm">
                <i class="bi bi-plus-lg"></i> Add New Department
            </a>
            <a href="<?php echo base_url . 'admin_dashboard.php?section=trash_departments'; ?>"
                ...
                <?php if (!empty($departments)): ?>
                    <?php foreach ($departments as $department): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars(date('Y-m-d H:i', strtotime($department['created_at']))); ?>
                            </td>
                            <td class="actions-column">
                                <div class="d-flex">
                                    <a href="<?php echo base_url . 'admin_dashboard.php?section=add_department&department_id=' . htmlspecialchars($department['department_id']); ?>"
                                        class="btn btn-sm btn-info text-white me-1">Edit</a>
                                    <button class="btn btn-sm btn-danger delete-department-btn"
    ...
    <nav aria-label="Page navigation">
        <div class="pagination-info-container">
            <p class="text-muted mb-0 fw-bold">Showing <?php echo $showingFrom; ?> to
                ...
                <?php echo $totalDepartments; ?> Departments
            </p>
            <ul class="pagination justify-content-center">
                ...
                if ($page > 1) {
                    echo '<li class="page-item"><a class="page-link" href="?section=departments&page=1&per_page=' . $itemsPerPage . '">First</a></li>';
                }

                // Show "Previous" page link
                if ($page > 1) {
                    echo '<li class="page-item"><a class="page-link" href="?section=departments&page=' . ($page - 1) . '&per_page=' . $itemsPerPage . '">Previous</a></li>';
                }
                ...
                for ($i = $startPageDisplay; $i <= $endPageDisplay; $i++): ?>
                    <li class="page-item <?php echo ($i == $page) ? 'active' : ''; ?>">
                        <a class="page-link"
                            href="?section=departments&page=<?php echo $i; ?>&per_page=<?php echo $itemsPerPage; ?>"><?php echo $i; ?></a>
                    </li>
                <?php endfor;
                ...
                // Show "Next" page link
                if ($page < $totalPages) {
                    echo '<li class="page-item"><a class="page-link" href="?section=departments&page=' . ($page + 1) . '&per_page=' . $itemsPerPage . '">Next</a></li>';
                }

                // Show "Last" page link
                if ($page < $totalPages) {
                    echo '<li class="page-item"><a class="page-link" href="?section=departments&page=' . $totalPages . '&per_page=' . $itemsPerPage . '">Last</a></li>';
                }
                ?>
            </ul>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                if (confirm('Are you sure you want to soft delete this department? It can be restored from Trash.')) {
                    // You'll need to create a new API endpoint for deleting departments
                    fetch('<?php echo base_url; ?>api/departments/delete_department_process.php', { // <--- **IMPORTANT: CREATE THIS FILE**
                        method: 'POST',
                        headers: {
                        ...
                        .catch(error => {
                            ...
                            alert('An error occurred while trying to delete the department. Check console for details.');
                        });
                }
            });
        });
