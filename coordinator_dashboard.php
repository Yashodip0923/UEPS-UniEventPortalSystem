...
// UniEventPortal Coordinator Dashboard - Displays events managed by the coordinator and related applications.
// NOTE: This is a basic display panel. For a real application, robust authentication
// and authorization (e.g., checking if $_SESSION['user_id'] is set and user_type is 'coordinator') are CRITICAL.

// Database connection include
require_once 'includes/db_connection.php'; // Apni database connection file include karein

// --- Simulate a logged-in coordinator user ID for testing ---
...
//     header('Location: login.php'); // Redirect to login if not authenticated or not a coordinator
//     exit();
// }
// $loggedInUserId = $_SESSION['user_id'];
$loggedInUserId = 2; // FOR TESTING: Use a dummy coordinator ID. Replace with actual session user ID in production.
// --- End Simulation ---

// Initialize arrays for data
$userDetails = [];
$managedEvents = [];
$eventApplications = []; // Applications for competitions within events managed by this coordinator
$error_message = '';

try {
    // 1. Fetch Coordinator User Details
    ...
    if (!$userDetails || $userDetails['user_type'] !== 'coordinator') {
        $error_message = "Coordinator not found or invalid user ID/type.";
        // Log out or redirect if user doesn't exist or isn't a coordinator
        // header('Location: logout.php'); exit();
    }

    // 2. Fetch Events managed by this coordinator
    $stmt = $pdo->prepare("
        SELECT 
            e.*, 
            d.department_name, 
        ...
        LEFT JOIN college_coordinators cc ON coord.user_id = cc.user_id
        LEFT JOIN colleges c ON cc.college_id = c.college_id
        WHERE e.coordinator_user_id = ?
        GROUP BY e.event_id -- Use GROUP BY to avoid duplicate rows if a coordinator is linked to multiple colleges (though unlikely for event creation)
        ORDER BY e.event_date DESC
    ");
    $stmt->execute([$loggedInUserId]);
    $managedEvents = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // 3. Fetch Competition Applications for competitions within events managed by this coordinator
    $stmt = $pdo->prepare("
        SELECT 
            ca.*, 
    ...
    // For production, log this error: error_log("Coordinator Dashboard data fetch error: " . $e->getMessage());
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Coordinator Dashboard - UniEventPortal</title>
    <!-- Bootstrap 5 CSS CDN -->
    ...
    <style>
        ...
        .card,
        .table-responsive {
            /* Renamed student-section to coordinator-section */
        ...
        .coordinator-section.hidden {
            /* Renamed student-section to coordinator-section */
            display: none !important;
        }

        /* Chart Canvas specific styling to control height - no charts for coordinator dashboard initially */
        canvas {
            max-height: 300px;
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <div class="container-fluid">
            ...
            <a class="navbar-brand d-flex align-items-center" href="admin_panel.php">
                ...
                <span class="fs-4 fw-bold text-primary">Coordinator</span>
            </a>
        </div>
    </nav>
    ...
    <div class="d-flex" id="wrapper">
        ...
        <div class="bg-dark border-right" id="sidebar-wrapper">
            ...
            <div class="list-group list-group-flush">
                ...
                <a href="#managedEvents" class="list-group-item list-group-item-action bg-dark text-white">My Managed
                    Events</a>
                <a href="#eventApplications" class="list-group-item list-group-item-action bg-dark text-white">Event
                    Applications</a>
        ...
        <div id="page-content-wrapper">
            <main class="container-fluid py-5 flex-grow-1">
                <h1 class="display-4 fw-bold text-center mb-5">Coordinator Dashboard</h1>
                ...
                <!-- Dashboard Section -->
                <div class="coordinator-section" id="dashboard">
                    <h2 class="h3 fw-bold mb-4">Dashboard Overview</h2>
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            <div class="card shadow-sm h-100 p-3">
                                <h5 class="card-title">Total Managed Events</h5>
                                <p class="display-4 text-primary fw-bold text-center">
                                    <?php echo count($managedEvents); ?>
                                </p>
                            </div>
                        </div>
                        <div class="col-md-6 mb-4">
                            <div class="card shadow-sm h-100 p-3">
                                <h5 class="card-title">Total Competition Applications Received</h5>
                                <p class="display-4 text-info fw-bold text-center">
                                    <?php echo count($eventApplications); ?>
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="coordinator-section mb-4 hidden" id="managedEvents">
                    <h2 class="h3 fw-bold mb-4">My Managed Events</h2>
                    <div class="d-flex justify-content-end mb-3">
                        <a href="#" class="btn btn-success btn-sm rounded-pill px-3">+ Create New Event</a>
                    </div>
                                    ...
                                    <?php foreach ($managedEvents as $event): ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                if ($event['is_competition_event'])
                                                    echo 'danger';
                                                else if ($event['is_departmental'])
                                                    echo 'primary';
                                                else
                                                    echo 'success';
                                                ?>">
                                            ...
                                            <td><?php echo htmlspecialchars($event['department_name'] ?? 'N/A'); ?></td>
                                            <td>
                                                <button class="btn btn-sm btn-info text-white me-1">Edit</button>
                                                <button class="btn btn-sm btn-primary me-1">View Registrations</button>
                                                <?php if ($event['is_competition_event']): ?>
                                                    <button class="btn btn-sm btn-secondary">Manage Competitions</button>
                                                <?php endif; ?>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                    ...
                    <?php else: ?>
                        <div class="alert alert-info text-center" role="alert">
                            You are not currently managing any events.
                        </div>
                    <?php endif; ?>
                </div>

                <!-- Event Applications Section -->
                <div class="coordinator-section mb-4 hidden" id="eventApplications">
                    <h2 class="h3 fw-bold mb-4">Competition Applications Received</h2>
                    <?php if (!empty($eventApplications)): ?>
                        <div class="table-responsive">
                            <table class="table table-hover table-striped">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    <?php foreach ($eventApplications as $app): ?>
                                        <tr>
                    ...
                    <?php else: ?>
                        <div class="alert alert-info text-center" role="alert">
                            No competition applications received for your managed events.
                        </div>
                    <?php endif; ?>
                </div>

                <!-- Profile Section -->
                <div class="coordinator-section mb-4 hidden" id="profile">
                ...
                <div class="coordinator-section hidden" id="settings">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const coordinatorSections = document.querySelectorAll('.coordinator-section'); // All content sections

            // Function to show a specific section and hide others
            function showSection(sectionId) {
                coordinatorSections.forEach(section => {
            ...
            // No charts needed on coordinator dashboard initially.
        });
    </script>
</body>
